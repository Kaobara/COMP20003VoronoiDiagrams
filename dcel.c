...
typedef struct vertex coordinate_t;
...
void getBisectorPoint(double distance, struct bisector *b, double *x, double *y){
    /* Fill in */
    if(b->isVertical == 1) {
        *x = b->bisectMidpoint.x;
        *y = b->bisectMidpoint.y + distance;
    } else {
        *x = b->bisectMidpoint.x + distance;
        *y = (b->gradient) * ((*x) - b->bisectMidpoint.x) + b->bisectMidpoint.y;
    }
...
struct bisector *getBisector(double x1, double y1, double x2, double y2){
    ...
    assert(p);
    /* Get the coordinates of the midpoint between points s1 and s2 */
    double midpointX = (x1 + x2) / 2.0;
    double midpointY = (y1 + y2) / 2.0;    

    p->bisectMidpoint.x = midpointX;
    p->bisectMidpoint.y = midpointY;

    if(y1 == y2) {
        p->isVertical = 1;
    } else {
        // if points is not vertical, it has a gradient
        p->isVertical = 0;
...
char *getIntersectionString(struct intersection *intersection){
    ...
    char *returnString = NULL;
    // Intersection depends on the direction of the edges
    if(intersection->startEdgeIndex <= intersection->endEdgeIndex){
        /* Find out memory needed. */
        int stringLength = snprintf(returnString, 0, 
            "From Edge %d (%lf, %lf) to Edge %d (%lf, %lf)", 
            intersection->startEdgeIndex, intersection->startVertex.x, intersection->startVertex.y,
            intersection->endEdgeIndex, intersection->endVertex.x, intersection->endVertex.y);
        returnString = (char *) malloc(sizeof(char) * (stringLength + 1));
        assert(returnString);
...
struct intersection *getIntersection(struct bisector *b, struct DCEL *dcel, int face,
    double minLength){
    double x1, y1, x2, y2;
    enum intersectType checkEdge1 = DOESNT_INTERSECT;
    enum intersectType checkEdge2 = DOESNT_INTERSECT;

    struct intersection *edgeIntersect;
    edgeIntersect = (struct intersection *)malloc(sizeof(struct intersection));
    assert(edgeIntersect);
    ...
    while(start != current || first) {
        ...
        if(checkEdge1 == INTERSECT || checkEdge1 == ENDS_OVERLAP) {
                checkEdge2 = intersects(current, b, dcel, minLength, &x2, &y2);

                if(checkEdge2 == INTERSECT) {
                    edgeIntersect->endEdgeIndex = current->edge;
                    edgeIntersect->endVertex.x = x2;
                    edgeIntersect->endVertex.y = y2;
                }
        }

        if(checkEdge1 == DOESNT_INTERSECT) {
            checkEdge1 = intersects(current, b, dcel, minLength, &x1, &y1);

            if(checkEdge1 == INTERSECT || checkEdge1 == ENDS_OVERLAP) {
                edgeIntersect->startEdgeIndex = current->edge;
                edgeIntersect->startVertex.x = x1;
                edgeIntersect->startVertex.y = y1;
            }
        }
    ...
    if((checkEdge1 == INTERSECT || checkEdge1 == ENDS_OVERLAP) &&  checkEdge2 == INTERSECT) {
...
double getDiameter(struct DCEL *dcel, int faceIndex){
    ...
    double x1, y1, x2, y2;
    struct halfEdge *firstStart = (dcel->faces)[faceIndex].he;
    struct halfEdge *secondStart = firstStart->next;
    int firstInitalLoop = 1;
    int secondInitalLoop = 1;
    
    struct halfEdge *firstCurrent = firstStart;
    struct halfEdge *secondCurrent = secondStart;
    while(firstStart != firstCurrent || firstInitalLoop) {
        x1 = (dcel->vertices)[firstCurrent->startVertex].x;
        y1 = (dcel->vertices)[firstCurrent->startVertex].y;
            while(secondStart != secondCurrent || secondInitalLoop) {
                x2 = (dcel->vertices)[secondCurrent->startVertex].x;
                y2 = (dcel->vertices)[secondCurrent->startVertex].y;
                currentDiameter = sqrt(((x2 - x1)*(x2 - x1)) + ((y2-y1)*(y2-y1)));
        ...
        // printf("Checking edges %d, %d in face %d\n", firstCurrent->edge, secondCurrent->edge, firstCurrent->face);
        secondInitalLoop = 1;
...
void incrementalVoronoi(struct DCEL *dcel, struct watchtowerStruct *wt){
    ...
    struct watchtowerStruct *pairedWT;

    // Find which face the new watchtower belongs to.
    // First watchtower placed into voronoi
    if((dcel->faces)[0].wt == NULL) {
        dcel->facesUsed = 1;
        (dcel->faces)[0].wt = wt;
        wt->face = 0;
        return;
    ...
    // Otherwise, there's already a watchtower, so just add.
    ...
    // Find which tower is also in the same face
    pairedWT = (dcel->faces)[initialFaceIndex].wt;

    /* Find the bisector between watchtower that is in the same face, 
    and find intersection of polygon with inital bisector. */
    struct bisector *bisectWatchtower = NULL;
    bisectWatchtower = getBisector(pairedWT->x, pairedWT->y, wt->x, wt->y);
    struct intersection *intersection = NULL;
    intersection = getIntersection(bisectWatchtower, dcel, initialFaceIndex, DEFAULTMINLENGTH);
    
    struct split tempSplit;
    if(getRelativeDir(wt->x, wt->y, &intersection->startVertex, &intersection->endVertex) == OUTSIDE) {
        tempSplit.startSplitPoint = intersection->startVertex;
        tempSplit.endSplitPoint = intersection->endVertex;
        tempSplit.startEdge = intersection->startEdgeIndex;
        tempSplit.endEdge = intersection->endEdgeIndex;
        tempSplit.verticesSpecified = 1;
    } else if(getRelativeDir(wt->x, wt->y, &intersection->startVertex, &intersection->endVertex) == INSIDE) {
        tempSplit.startSplitPoint = intersection->endVertex;
        tempSplit.endSplitPoint = intersection->startVertex;
        tempSplit.startEdge = intersection->endEdgeIndex;
        tempSplit.endEdge = intersection->startEdgeIndex;
        tempSplit.verticesSpecified = 1;
    }
    applySplit(&tempSplit, dcel);
    (dcel->faces)[dcel->facesUsed-1].wt = wt;
    wt->face = dcel->facesUsed-1;
    
    // Free current bisector and intersections before next malloc
    if(bisectWatchtower != NULL) {
        freeBisector(bisectWatchtower);
        bisectWatchtower = NULL;
    }
    ...
    struct halfEdge *initialBisectHalfEdge = dcel->faces[dcel->facesUsed-1].he;
    struct halfEdge *currHalfEdge = initialBisectHalfEdge->next;
    int fullRotation = 0;
    // Start going clockwise to check for voronoi edges, and start splitting
    while(currHalfEdge->pair!=NULL) {
        pairedWT = (dcel->faces)[currHalfEdge->pair->face].wt;
        if(pairedWT == NULL) {
            break;
        }
        
        bisectWatchtower = getBisector(pairedWT->x, pairedWT->y, wt->x, wt->y);
        intersection = getIntersection(bisectWatchtower, dcel, pairedWT->face, DEFAULTMINLENGTH);
        
        struct split tempSplit;
        if(getRelativeDir(wt->x, wt->y, &intersection->startVertex, &intersection->endVertex) 
                == OUTSIDE) {
            tempSplit.startSplitPoint = intersection->startVertex;
            tempSplit.endSplitPoint = intersection->endVertex;
            tempSplit.startEdge = intersection->startEdgeIndex;
            tempSplit.endEdge = intersection->endEdgeIndex;
            tempSplit.verticesSpecified = 1;
        } else if(getRelativeDir(wt->x, wt->y, &intersection->startVertex, &intersection->endVertex) 
                == INSIDE) {
            tempSplit.startSplitPoint = intersection->endVertex;
            tempSplit.endSplitPoint = intersection->startVertex;
            tempSplit.startEdge = intersection->endEdgeIndex;
        ...
        if(intersection->startEdgeIndex == initialBisectHalfEdge->edge) {
            fullRotation = 1;
            break;
        }
    }
    ...
    // Same as above, but anticlockwise. If it already went through all edges, 
    // then just skip (full rotation)
    currHalfEdge = initialBisectHalfEdge->prev;
    while(currHalfEdge->pair!=NULL && fullRotation == 0) {
        pairedWT = (dcel->faces)[currHalfEdge->pair->face].wt;
        ...
        currHalfEdge = currHalfEdge->prev;
    }


    // Clean up
    currHalfEdge = initialBisectHalfEdge;
    int firstClean = 1;
    int disconnectEdgeIndex;
    int facesCleaned = 0;
    while(currHalfEdge->next != initialBisectHalfEdge || firstClean) {
        if(currHalfEdge->next->pair != NULL && currHalfEdge->next->pair->face > wt->face) {
            disconnectEdgeIndex = currHalfEdge->next->edge;
            currHalfEdge->next = currHalfEdge->next->pair->next;
            currHalfEdge->next->prev = currHalfEdge;

            while(currHalfEdge->next->edge != disconnectEdgeIndex) {
                // if(currHalfEdge->next->pair && currHalfEdge->next->pair->face > wt->face) {
                //     // In the case that the face doesn't loop directly back to the same edge
        ...
        currHalfEdge->face = wt->face;
        currHalfEdge = currHalfEdge->next;
        firstClean = 0;
    }

    dcel->facesUsed -= facesCleaned;
    // Free current bisector and intersections before next malloc
    if(bisectWatchtower != NULL) {
        freeBisector(bisectWatchtower);
        bisectWatchtower = NULL;
